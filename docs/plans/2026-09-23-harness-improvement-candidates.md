...
**G1 结果与排序更新（2026-09-25，详见 `docs/reports/2026-09-25-offline-g1-glm.md`）**

按预先登记的判据：

- 通过：T4 覆盖（E3 为 9/11，安慰剂 1/11；E3b 为 10/11；E3c 取回 7/11），以及 A2 新上下文撰写（E10：归因错误组多数成功 7 对 5，原成功不下降，并且优于 F1）。
- 淘汰：F1 单独使用（E4）、F2（EJ）、S1/S2（E8）、R3 thinking（E5）。
- 工程项 4 离线非劣：最后一步 prompt token 分别 −37.2%（只做标签合并）与 −54.3%（再加折叠）。
- L6 有问题证据：不加收口指令时，7.4% 的输出没有结论。
- 更新后的 live 顺序：T4b + F1（network_policy 臂，按预先登记规则），A2（归因臂）。
- 限制：glm judge 无法区分 NML 的定位错误（E1g 中 5/5 条官方失败都被判为成功），A2 的 live 设计需要在 G4 之前修订。
